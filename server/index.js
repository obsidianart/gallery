...
app.get('/api/list', requestDelay, async (req, res) => {
...
app.delete('/api/delete/:id', requestDelay, async (req, res) => {
  const result = await docs.delete(req.params.id)
  res.send(result) //Idempotent
})

app.post('/api/upload', requestDelay, async (req, res) => {
